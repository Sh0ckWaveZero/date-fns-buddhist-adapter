...
- Excluded `*.map` source maps from the published package (they are only useful when debugging into `node_modules`; `.d.ts` type definitions are unaffected). Local builds still generate them for the `file:..` linked example
